              ...
              <a-dropdown :getPopupContainer="(trigger) => trigger">
                ...
                <a-menu slot="overlay">
                  ...
                  <template v-if="treeNodeData.key !== 'all'">
                    ...
                    <!-- <a-menu-item
                      v-if="treeNodeData.isSubnet"
                      @click="handleScanSubnet(treeNodeData)"
                    > -->
                    <a-menu-item
                      @click="handleScanSubnet(treeNodeData)"
                    >
                      <a>
                        <a-icon
                          type="wifi"
                          :class="['ipam-tree-node-menu-icon', scanningCiId === treeNodeData._id ? 'spin' : '']"
                        />
                        {{ $t('cmdb.ipam.scanNow') }}
                      </a>
                    </a-menu-item>
...
import { deleteIPAMSubnet, deleteIPAMScope, triggerSubnetScan } from '@/modules/cmdb/api/ipam.js'
...
export default {
  ...
  methods: {
    ...
    handleScanSubnet(node) {
      if (this.scanningCiId === node._id) {
        this.$message.info(this.$t('cmdb.ipam.scanningTip'))
        return
      }
      this.$confirm({
        title: this.$t('tip'),
        content: this.$t('cmdb.ipam.scanConfirmTip', { cidr: node.cidr }),
        onOk: () => {
          this.scanningCiId = node._id
          triggerSubnetScan(node._id)
            .then(() => {
              this.$message.success(this.$t('cmdb.ipam.scanStartedTip'))
            })
            .catch(() => {
              this.$message.error(this.$t('cmdb.ipam.scanFailedTip'))
            })
            .finally(() => {
              // 延迟清除状态，给用户一段时间的防重复点击保护
              setTimeout(() => { this.scanningCiId = null }, 30000)
            })
        }
      })
    }
