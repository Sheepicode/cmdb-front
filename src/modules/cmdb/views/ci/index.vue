              ...
              >
                ...
                <a-dropdown>
                  ...
                  <a-menu slot="overlay">
                    ...
                    <!-- 新增：同名属性回填 -->
                    <a-menu-item key="syncRelationAttrs" @click="handleSyncRelationAttrs">
                      <a-icon type="sync" :spin="isSyncing" />
                      {{ isSyncing ? $t('cmdb.preference.syncingRelationAttrs') : $t('cmdb.preference.syncRelationAttrs') }}
                    </a-menu-item>
                  </a-menu>
                </a-dropdown>
              </div>
...
import { syncRelationAttrs } from '@/modules/cmdb/api/ci'
...
export default {
  ...
  methods: {
    ...
    handleSyncRelationAttrs() {
      if (this.isSyncing) {
        this.$message.info(this.$t('cmdb.preference.syncingRelationAttrs'))
        return
      }
      this.$confirm({
        title: this.$t('tip'),
        content: this.$t('cmdb.preference.syncRelationAttrsConfirm'),
        onOk: () => {
          this.isSyncing = true
          syncRelationAttrs(this.currentTypeId)
            .then((res) => {
              const { updated, conflicts } = res
              if (conflicts > 0) {
                this.$message.warning(
                  this.$t('cmdb.preference.syncRelationAttrsConflict', { updated, conflicts })
                )
              } else {
                this.$message.success(
                  this.$t('cmdb.preference.syncRelationAttrsSuccess', { updated })
                )
              }
              if (updated > 0) {
                this.reloadData()
            ...
            .catch(() => {
              this.$message.error(this.$t('cmdb.preference.syncRelationAttrsFailed'))
            })
            .finally(() => {
              // 延迟清除，防止后端还在处理时重复触发
              setTimeout(() => { this.isSyncing = false }, 5000)
            })
        },
      })
    }
