            ...
            >
              ...
              >
                <div
                  ...
                  :key="nameIndex"
                  class="rack-container-main-list-device-name with-power"
                  @click.stop="openDeviceDetail(nameItem)"
                >
                  <div class="device-power">
                    <span
                      class="power-dot"
                      :class="{ on: item.power_supply && item.power_supply.includes('A') }"
                    > A</span>
                    <span
                      class="power-dot"
                      :class="{ on: item.power_supply && item.power_supply.includes('B') }"
                    >B</span>
                  </div>
                  <CIIcon size="14" :icon="nameItem.icon" />
                  <a-tooltip placement="topLeft" :title="nameItem.name || '未命名'">
...
export default {
  ...
  methods: {
    ...
    migrateDevice(data) {
      this.$emit('migrateDevice', data.id, data.power_supply)
    },
...
.rack-container-main-list-device-name.with-power {
  display: flex;
  align-items: center;
}
...
.device-power .power-dot {
  width: 14px;
  height: 14px;
  border-radius: 50%;
  background-color: #C9CDD4;   // 默认灰
  color: #fff;
  font-size: 10px;
  font-weight: 600;
  line-height: 14px;
...
.device-power .power-dot.on {
  background-color: #00C853;   // 绿灯
